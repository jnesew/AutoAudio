...
from core.config import AppConfig, TTSConfig
...
class ResumePipelineIntegrationTests(unittest.TestCase):
    ...
    def test_resume_after_interrupted_conversion_uses_checkpointed_segments(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            provider_args = self._build_args(input_book=input_book, output_dir=output_dir, resume="yes")
            provider_args.replacement_file = str(replacement_file)
            provider_args.replacement_rules = ["one=won"]
            with self.assertRaisesRegex(ResumeStateError, "no compatible checkpoint state exists"):
                run_pipeline(provider_args, provider_config)
